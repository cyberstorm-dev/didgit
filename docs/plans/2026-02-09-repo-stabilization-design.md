...
**Scope update:** Target chains are **Base mainnet (default)** and **Arbitrum One mainnet**. All chain-dependent values must be parameterized and cataloged per chain (no UID parity assumptions).
...
Chain configuration will be centralized into explicit per-chain blocks (e.g., `chains.base`, `chains.arbitrum`) with:
- `chainId`
- `rpcUrl` or `rpcUrls`
- `easAddress`
- `schemaUids` (identity, contribution, permission, repo globs)
- explorer URLs (basescan/arbiscan, easscan equivalents)

Default chain is Base; override via `CHAIN=arbitrum` or equivalent config entrypoint. No assumption that schema UIDs are identical across chains.
...
5. **Chain parameterization**
   - Central config object with per-chain addresses + schema UIDs.
   - Default chain = Base; switchable by env/flag.
...
Chain selection flows from a single config source (env/flag) to all components needing chain-specific values (backend, worker, docs, CLI tooling).
...
3. Add chain parameterization + update docs for Base + Arbitrum.
4. Fix remaining build/test failures with TDD and minimal changes.
...
- Chain configuration is explicit and defaults to Base, with Arbitrum support via flag.
