/**
 * Test with FRESH Kernel account that has permission validator from creation
 */
...
async function test() {
  ...
  console.log('=== Fresh Kernel with Permission Test ===\n');
  ...
  console.log('Step 1: Creating fresh Kernel with permission validator...');
  ...
  // Create Kernel with BOTH validators
  ...
  console.log('Step 4: Backend creates attestation using permission...');

  // Backend recreates permission validator (same config)
  ...
  // Reference existing Kernel with permission validator
  ...
  const contributionData = encodeAbiParameters(
    ...
    [
      ...
      'test-fresh-kernel-' + Date.now(),
      'test-user',
      'Test: fresh Kernel with permission validator from creation',
  ...
  console.log('   Kernel created with permission validator');
