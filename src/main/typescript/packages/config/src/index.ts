import { z } from 'zod';

const envSchema = z.object({
  VITE_EAS_BASE_SEPOLIA_SCHEMA_UID: z.string().startsWith('0x').length(66).optional(),
  VITE_EAS_BASE_SEPOLIA_ADDRESS: z.string().optional(),
  VITE_EAS_SCHEMA_UID: z.string().startsWith('0x').length(66).optional(),
  VITE_EAS_ADDRESS: z.string().optional(),
  VITE_ZERODEV_PROJECT_ID: z.string().optional(),
  VITE_ZERODEV_BUNDLER_RPC: z.string().url().optional(),
});

export type AppConfig = ReturnType<typeof appConfig>;

export function appConfig() {
  const parsed = envSchema.safeParse((globalThis as any).import?.meta?.env ?? process.env ?? {});
  const fallbackSchema = '0x7e4a502d6e04b8ff7a80ac8b852c8b53199fe297ddf092a63fffb2a5a062b1b7';
  if (!parsed.success) {
    return {
      EAS_SCHEMA_UID: fallbackSchema,
      EAS_ADDRESS: undefined,
      ZERODEV_PROJECT_ID: undefined,
      ZERODEV_BUNDLER_RPC: undefined,
    } as const;
  }
  const EAS_SCHEMA_UID = parsed.data.VITE_EAS_BASE_SEPOLIA_SCHEMA_UID ?? parsed.data.VITE_EAS_SCHEMA_UID ?? fallbackSchema;
  let EAS_ADDRESS = parsed.data.VITE_EAS_BASE_SEPOLIA_ADDRESS ?? parsed.data.VITE_EAS_ADDRESS;
  if (EAS_ADDRESS && (EAS_ADDRESS.trim().toLowerCase() === 'undefined' || EAS_ADDRESS.trim() === '')) {
    EAS_ADDRESS = undefined as any;
  }
  return {
    EAS_SCHEMA_UID,
    EAS_ADDRESS,
    ZERODEV_PROJECT_ID: parsed.data.VITE_ZERODEV_PROJECT_ID,
    ZERODEV_BUNDLER_RPC: parsed.data.VITE_ZERODEV_BUNDLER_RPC,
  } as const;
}

